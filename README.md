...
- 仕様書
  - ファイル名の例 local_notebook_ai_sdd_spec_v1_6.md
- 実装プロンプト集
  - ファイル名の例 local_notebook_ai_implementation_prompts_v1_6.md
...
改善しない場合は、他のモデルへの切り替えも検討してください（`granite4.2:3b`
＋`OLLAMA_DISABLE_THINKING=true`の組み合わせでは安定した回答が確認できています）。

### なぜ「思考（Think）」モードが問題になるのか

Granite・Qwen3等の一部モデルは、回答を出す前に内部で考える過程（Think／thinking）
を生成する仕組みを持っています。Ollamaの素の補完エンドポイント（`/api/generate`）
をこのモードのまま呼び出すと、モデルが**要求した出力（HyDE仮想文書や回答本文）の
代わりに、その内部の思考過程そのものを返してしまう**ことがあります。実際に
`granite4.2:3b`で検証した際、"We need to respond as a document generator..."
のような英語の思考テキストがそのまま返ってくる事象を観測しました。

これが起こると、以下の問題につながります。

1. HyDE仮想文書が本来の300〜600文字程度ではなく、思考テキストのぶん
   **異常に長く**なる
2. その長すぎるテキストをそのままEmbeddingモデルに渡そうとして、
   `the input length exceeds the context length`というエラーでRAGが失敗する
3. 長さが上限を超えなくても、要求した内容とは違う中身（思考の跡）が
   検索クエリや回答生成の材料として使われてしまい、意図した動作にならない

これを防ぐため、`OLLAMA_DISABLE_THINKING=true`のときはOllamaへのリクエストの
**トップレベル**に`think: false`を送っています（`temperature`や`num_predict`
のように`options`オブジェクトの中に入れるパラメータとは扱いが異なる点に注意）。
これによりモデルに思考モードを使わせず、要求した出力だけを返させます。

ただしこれはOllama・モデルのバージョンに依存する挙動で、効果が無い場合もあります。
`OLLAMA_DISABLE_THINKING=true`にしても改善しない場合は、モデル自体の変更も
検討してください。
